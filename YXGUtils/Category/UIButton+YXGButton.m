...
@implementation UIButton (YXGButton)
...
{
    objc_setAssociatedObject(self, @selector(yxg_addActionHandler:), touchHandler, OBJC_ASSOCIATION_COPY_NONATOMIC);
...
{
    TouchedButtonBlock block = objc_getAssociatedObject(self, @selector(yxg_addActionHandler:));
    if (block)
    {
        block();
    }
