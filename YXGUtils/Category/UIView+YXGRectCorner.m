...
#import "UIView+YXGRectCorner.h"
#import <objc/runtime.h>

static NSString * const kcornerRadius = @"yxg_rectCornerRadius";
static NSString * const krectCorner = @"yxg_rectCorner";

@implementation UIView (YXGRectCorner)

- (void)setYxg_Radius:(CGFloat)yxg_Radius{
    CGFloat Radius = [objc_getAssociatedObject(self, &kcornerRadius) floatValue];
    if (Radius != yxg_Radius) {
        [self willChangeValueForKey:kcornerRadius];
        objc_setAssociatedObject(self, &kcornerRadius, @(yxg_Radius), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        [self didChangeValueForKey:kcornerRadius];
        [self yxg_rectCornerWithCornerRadius:yxg_Radius Corner:self.yxg_RectCorner];
    }
}
- (CGFloat)yxg_Radius{
    if (!objc_getAssociatedObject(self, &kcornerRadius)) {
        [self setYxg_Radius:5];  // 设置默认值
    }
    return [objc_getAssociatedObject(self, &kcornerRadius) floatValue];
}

- (void)setYxg_RectCorner:(UIRectCorner)yxg_RectCorner{
    [self willChangeValueForKey:krectCorner];
    objc_setAssociatedObject(self, &krectCorner, @(yxg_RectCorner), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    [self didChangeValueForKey:krectCorner];
    
    [self yxg_rectCornerWithCornerRadius:self.yxg_Radius Corner:yxg_RectCorner];
}
- (UIRectCorner)yxg_RectCorner{
    return [objc_getAssociatedObject(self, &krectCorner) intValue];
}

/// 内部方法
- (void)yxg_rectCornerWithCornerRadius:(CGFloat)cornerRadius Corner:(UIRectCorner)corner{
...
- (void)yxg_GradientBgColorWithColors:(NSArray *)colors Locations:(NSArray *)locations StartPoint:(CGPoint)startPoint EndPoint:(CGPoint)endPoint{
    CAGradientLayer *gradientLayer = [self yxg_GradientLayerWithColors:colors Frame:self.bounds Locations:locations StartPoint:startPoint EndPoint:endPoint];
